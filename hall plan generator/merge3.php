...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['file1']) && isset($_FILES['file2']) && isset($_FILES['file3'])) {
        ...
        $spreadsheet3 = $reader->load($_FILES['file3']['tmp_name']);
        ...
        // Get the highest row from all sheets
        $rowCount1 = $sheet1->getHighestRow();
        $rowCount2 = $sheet2->getHighestRow();
        $rowCount3 = $sheet3->getHighestRow();
        ...
        // Copy data from the third file, starting after the last row of the second file
        for ($row = 6; $row <= $rowCount3; $row++) {
            for ($col = 'A'; $col <= $highestColumn; $col++) {
                $cellValue = $sheet3->getCell("$col$row")->getValue();
                $mergedSheet->setCellValue("$col" . ($row + $rowCount1 + $rowCount2 - 10), $cellValue);

                // Enable text wrapping for the cell
                $mergedSheet->getStyle("$col" . ($row + $rowCount1 + $rowCount2 - 10))->getAlignment()->setWrapText(true);
            }
        }
        ...
        echo "Please upload all three files.";
    }
}
?>
