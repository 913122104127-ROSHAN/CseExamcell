...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $secondYearLabel = $_POST['secondYearLabel'];
    $thirdYearLabel = $_POST['thirdYearLabel'];
    $secondYearFile = $_FILES['secondYearFile']['tmp_name'];
    $thirdYearFile = $_FILES['thirdYearFile']['tmp_name'];
    
 // Extract data for second set
 $fourthYearFile = isset($_FILES['fourthYearFile']['tmp_name']) ? $_FILES['fourthYearFile']['tmp_name'] : null;
 $fifthYearFile = isset($_FILES['fifthYearFile']['tmp_name']) ? $_FILES['fifthYearFile']['tmp_name'] : null;
 $hallNames = array_map('trim', explode(',', $_POST['hallNames'])); // Extract hall names
    ...
    $secondYearFileName = pathinfo($_FILES['secondYearFile']['name'], PATHINFO_FILENAME);
    $thirdYearFileName = pathinfo($_FILES['thirdYearFile']['name'], PATHINFO_FILENAME);
    
       // Optional: Handle filenames for the second set
       $fourthYearFileName = $fourthYearFile ? pathinfo($_FILES['fourthYearFile']['name'], PATHINFO_FILENAME) : null;
       $fifthYearFileName = $fifthYearFile ? pathinfo($_FILES['fifthYearFile']['name'], PATHINFO_FILENAME) : null;
    // Create the dynamic output file name
    $outputFileName = "{$secondYearFileName} & {$thirdYearFileName} Hall Plan.xlsx";
    $outputFileNameSecondSet = "{$fourthYearFileName} & {$fifthYearFileName} Hall Plan.xlsx";
    $reader = new ReaderXlsx();

    // Load Second Year Data
    $spreadsheet = $reader->load($secondYearFile);
    $secondYearData = $spreadsheet->getActiveSheet()->toArray();
    $secondYearSections = splitSections($secondYearData);

    // Load Third Year Data
    $spreadsheet = $reader->load($thirdYearFile);
    ...
    if ($fourthYearFile && $fifthYearFile) {
        $spreadsheet = $reader->load($fourthYearFile);
        $fourthYearData = $spreadsheet->getActiveSheet()->toArray();
        $fourthYearSections = splitSections($fourthYearData);

        $spreadsheet = $reader->load($fifthYearFile);
        $fifthYearData = $spreadsheet->getActiveSheet()->toArray();
        $fifthYearSections = splitSections($fifthYearData);
    }
    ...
    if ($fourthYearFile && $fifthYearFile) {
        list($hallAllocationsSecondSet, $remainingFourthYearQueue, $remainingFifthYearQueue) = allocateStudentsToHalls(
            $hallNames,
            $hallCapacities,
            $fourthYearSections,
            $fifthYearSections
    ...
    foreach ($hallAllocations as $allocation) {
        ...
        $sheet->setCellValue('B' . $row, $secondYearLabel);
        ...
        $sheet->setCellValue('A' . $row, $allocation['Hall']);
        $sheet->setCellValue('B' . $row, $thirdYearLabel);
    ...
    if (!empty($remainingSecondYearQueue) || !empty($remainingThirdYearQueue)) {
        if (!empty($remainingSecondYearQueue)) {
            ...
            $sheet->setCellValue("A{$row}", "Remaining ".$secondYearFileName);
            $sheet->setCellValue("B{$row}", $secondYearLabel);
        ...
        if (!empty($remainingThirdYearQueue)) {
            ...
            $sheet->setCellValue("A{$row}", "Remaining ".$thirdYearFileName);
            $sheet->setCellValue("B{$row}", $thirdYearLabel);
...
$outputFileName = $directory . "{$secondYearFileName} & {$thirdYearFileName} Hall Plan.xlsx";
